...
| CrowdWorks | **The real form blocker is now named; application acceptance pending.** Release `3a3b93ae` started the shared judge successfully, selected job `13439041`, and reached its official proposal surface. That job is a competition whose form requires a finished artifact upload before contract, not the fixed-price form implemented by this adapter; it was misreported as `proposal_form_changed` and stopped the wake. The adapter now classifies that provider workflow explicitly and continues to the next job; 27 focused tests pass and production acceptance remains open. | **Historical effects accepted; current owner broken.** Installed release `460851bf` ends `entrypoint_exit_120`. The retained shared aggregate represents `observed=10`, `readback=10`, `failed=0`, `pending=0` and five replay-zero receipts, but it does not override the newer failed terminal. | **Not implemented.** The storefront owner remains disabled, so there is no listing, inquiry, order or revenue receipt chain. | **Broken and no revenue proof.** Installed release `75c600da` ends `entrypoint_exit_1`; current provider inventory fails before any real contract, delivery or payout receipt. |
