...
| L11 | Local 24/7 repeatability — **ACTIVE (restarted window: 1/30 NY days, 47/100 natural wakes)** | The main-core verifier reads immutable runtime events, the durable Telegram outbox, live receipts, and official broker orders. Its repeatability window passes by either 100 natural wakes or 30 consecutive New-York dates including a weekend; every path also requires multiple process IDs, one terminal event and one delivered Telegram report per natural wake, zero event/order/client-ID duplicates, zero unresolved effects, and shadow `effect_class=none`. The first window failed after 8 clean wakes when a host-wide process-spawn incident left many unrelated Apple/Life Manager jobs in `xpcproxy`; a target-only restart also reproduced it. That failed evidence remains immutable. The host later recovered without a Mac restart. The new consecutive window starts at the first complete post-failure execute event, `2026-09-09T22:56:19.380495Z`; production evidence reports Telegram `47/47`, processes `47`, official orders `2`, and all zero-duplicate/no-effect/unresolved checks pass. Safe cleanup removed no protected release and reports about 28 GB free. The gate remains incomplete until either 53 more natural wakes pass or 29 more consecutive days including a weekend are observed; fixture time cannot satisfy it. |
