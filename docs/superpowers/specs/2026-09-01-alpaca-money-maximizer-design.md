...
| L14 | Ownership transfer and cloud shadow parity — **ACTIVE** | The Cloud build prerequisite now installs only the official Alpaca CLI `v0.0.14` Linux artifact, pins both supported architecture checksums, fails the build on digest or version mismatch, and passes an actual download/hash/ELF fixture test. A new service-role-only, tenant-keyed runtime bundle store rejects raw credential fields, unknown files, cross-tenant rows, oversized payloads, and digest drift. Its tested exporter/importer migrates only the live account hash binding, pause/kill control, risk-day baseline/high-water state, receipt ledger (including unresolved intents and issued client IDs), live position ownership marker, and Telegram outbox/latest receipt with private modes; observation caches and credentials are excluded. Focused tests pass `7/7`, and commits `dc7f5ff4f`, `d381741d1`, and `bde97ccdc` are pushed. Before the first cloud wake, still stop local and read back no running pass, drain both queues, reconcile official orders/positions, persist and rehydrate the production bundle, then enable cloud shadow for at least two natural 300-second wakes. Fixture inputs must produce the same decision/risk/receipt shape and restart must add zero duplicate jobs or effects. |
