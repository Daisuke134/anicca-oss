...
| L14 | Ownership transfer and cloud shadow parity — **ACTIVE** | Before the first cloud wake, stop local and read back no running pass, drain both queues, reconcile official orders/positions, and verify export/import of account binding, pause/kill state, loss-day baseline/high-water state, receipts, unresolved intents, and issued client IDs. Only then enable cloud shadow for at least two natural 300-second wakes. Fixture inputs produce the same decision/risk/receipt shape and restart adds zero duplicate jobs or effects. |
