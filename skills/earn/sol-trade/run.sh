...
if [ -n "$SIGS_JSON" ]; then
  ...
  REC=$(env -i PATH="$PATH" HOME="$HOME" SOLANA_RPC_URL="${SOLANA_RPC_URL:-}" SIGS_JSON="$SIGS_JSON" WALLET="$OWN_WALLET" EARN_LEDGER="$LEDGER" WAKE_ID="${WAKE_ID:-$(date -u +%s)}" LIFE_MANAGER_REPO="${ANICCA_REPO:-$SKILL_DIR/../../..}" LM_FINANCIAL_RECORDS_DIR="${LM_FINANCIAL_RECORDS_DIR:-${CFO_STATE_DIR:-$HOME/.local/state/life-manager/life-manager-cfo-hourly}/financial-records}" LM_CFO_SUBJECT_ID="${LM_CFO_SUBJECT_ID:-${LM_UID:-local}}" LM_CFO_TELEGRAM_CHAT_ID="${LM_CFO_TELEGRAM_CHAT_ID:-${TELEGRAM_ALERT_CHAT_ID:-${LM_ADMIN_TELEGRAM_CHAT_ID:-}}}" LIFE_MANAGER_ENV_FILE="${LIFE_MANAGER_ENV_FILE:-$HOME/.local/state/life-manager/.env}" CFO_TELEGRAM_OUTBOX="${CFO_TELEGRAM_OUTBOX:-${CFO_STATE_DIR:-$HOME/.local/state/life-manager/life-manager-cfo-hourly}/telegram-outbox.sqlite3}" CFO_PYTHON_BIN="${CFO_PYTHON_BIN:-python3}" node "$SKILL_DIR/lib/record-swap.mjs" 2>/dev/null || true)
