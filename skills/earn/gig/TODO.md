...
| Mercor | **Active but not accepted.** The dedicated 30-minute owner is authenticated and scans official opportunities through an owner-scoped shared-browser context. It sends candidate-local human gates and continues scanning, but the current application ledger remains at 19 and no new official submit/readback has been proved; a candidate-detail traversal exception still prevents end-to-end completion. | **Partial code, no active lane.** Calendar/Gmail and human-gate components exist, but there is no installed Mercor Reply owner with official message readback. | **Not applicable today.** No seller storefront workflow is implemented or evidenced for Mercor. | **Code-ready and monitoring, not live-accepted.** A thin adapter enters the shared Paid kernel and preserves missing official work inventory as durable pending with failed zero. It refuses non-`work.mercor.com` evidence and keeps human-required submission as an explicit wait. The installed owner has one natural pending terminal with no external effect, but no real submission or settled payout receipt proves positive operation yet. |
...
7. [ ] `MERCOR-APPLY-1` Restore Mercor as an independent revenue-marketplace Apply owner, not as a
   ...
   human gates, sends the exact PDF job/link/action notification, and continues through four Explore
   pages. This proves the gate itself no longer ends the pass. It still has no application effect: the
   private application ledger remains at 19. The model then finds multiple `1-click apply` candidates
   and opens `Product / Platform Experts`, but its temporary multi-card detail traversal dereferences a
   missing card position (`TypeError: 'NoneType' object is not subscriptable`) before reaching a ready
   submit. This candidate-local failure is incorrectly consuming the bounded wake instead of being
   recorded and skipped. The atom remains unchecked. Next action is to make exact-card disappearance a
   recorded candidate-local outcome, continue to the next observed stable listing ID, and prove one
   official submit/readback, immediate shared Telegram receipt, then replay-zero on the parked context.
