...
| Mercor | **Matched application replay-zero passed; material-low submission defect is active.** PR `#5004` adds explicit shared high/medium/low definitions. Natural run `18d423d23f7c5250-80399` officially submitted the medium-fit `Software Engineer, Python — Codebase Q&A` listing with Telegram application receipt `75661`; following run `18d4261d3ee9d360-30246` kept that application and Telegram event at exactly one. However the replay wake then classified `Software Engineer, Go — Codebase Q&A` low because substantial production Go and stated seniority are unverified, yet submitted it and delivered Telegram `75740`. That contradicts the expected-acceptance objective. Repair the shared policy so a material required language/location/domain/seniority contradiction is `no_reasonable_shot` and skipped, while preferred/missing evidence remains rank-later and truthful medium/high submissions continue. | **Accepted.** Consecutive natural passes cover the authenticated in-platform/application event inventory; email is not treated as a platform reply. | **Only official empty-contract monitoring is proved.** Real task preparation, human handoff when physically required, submission, acceptance and payout remain unaccepted. | **Not applicable on current official surface.** | Applications exist, but no accepted paid work or payout is proved. |
...
The active atom is **Mercor Apply material-low submission prevention**. Lancers project
...
unverified. Tighten the one shared ranking contract at the material-contradiction boundary, then
require a natural wake showing low material contradictions skipped without blocking qualified work.
