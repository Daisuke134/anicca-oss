...
| Mercor | **Matched application replay-zero passed; material-low and shared tool-host repairs are installed pending natural acceptance.** PR `#5004` adds explicit shared high/medium/low definitions. Natural run `18d423d23f7c5250-80399` officially submitted the medium-fit `Software Engineer, Python — Codebase Q&A` listing with Telegram application receipt `75661`; following run `18d4261d3ee9d360-30246` kept that application and Telegram event at exactly one. That replay wake also exposed and submitted the low-fit `Software Engineer, Go — Codebase Q&A` listing with Telegram `75740`. PR `#5005`, merged as `fe149d2d`, makes material required contradictions non-submittable. Its first natural run `18d4294202e555b0-4482` ended process exit zero but business result `blocked`: the Codex code-mode host negotiation timed out before any tool action, inspection or application; Telegram `75840` reported it. Shared agent-runner PR `#5007`, merged as `8e0a1a4d`, now falls through to the configured provider fallback only for this exact pre-tool transient. Immutable release `20260911T133325-8e0a1a4d` is loaded only for Mercor Apply with receipt `0648d8f832b59c9b07cbac2b`; require another natural wake proving real inspection and the fit boundary. | **Accepted.** Consecutive natural passes cover the authenticated in-platform/application event inventory; email is not treated as a platform reply. | **Only official empty-contract monitoring is proved.** Real task preparation, human handoff when physically required, submission, acceptance and payout remain unaccepted. | **Not applicable on current official surface.** | Applications exist, but no accepted paid work or payout is proved. |
...
`0648d8f832b59c9b07cbac2b`. Require a natural wake showing real inspection and low material contradictions skipped without
