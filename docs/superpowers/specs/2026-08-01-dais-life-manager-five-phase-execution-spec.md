...
容量復旧後のmanual wake `wake-792c1ddd99381cdf735ff94e`はCalendarを通過し、Luma audit `observed 36 / normalized 6 / window 6 / free-open 3 / calendar-free 0`、Connpass audit `observed 295 / normalized 295 / window 295 / free-open 246 / calendar-free 19`を保存して候補処理へ進んだ。これはscheduler、Calendar readback、Luma/Connpass discovery、候補rotationの復帰証拠としてacceptする。一方、新規Luma official registration/pending、Calendar exact 1、PNG/receipt、Telegram positive IDs、durable bundleはまだ0であり、manual wakeはCG-44の自然wake acceptanceへ数えない。

したがって現行statusはConnector **NOT DONE（CG-44 active）**、Fundraiser **NOT DONE（固定順序で未着手）**、shared OSS components **PARTIAL / NOT DONE**である。残順序は`CG-44 live Luma bundle → CG-45 replay-zero → CG-47 LT → CG-48 natural duplicate/cleanup → CG-51 Connector closure → Fundraiser natural effect/replay-zero → Telegram OSS fresh-clone E2E → Cloud`から変更しない。次の一手は、実行中wakeのterminal・Telegram delivery・external effectをreadbackし、effectが無ければ次の30分natural batchを継続することである。
