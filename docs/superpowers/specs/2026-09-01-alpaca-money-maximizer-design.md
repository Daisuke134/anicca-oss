...
| L07 | Minimal Telegram reporting — **ACTIVE** | The shared reporter now receives the same durable risk snapshot as the entry gate and renders every wake with action/`NO_TRADE`, reason, equity, cash, conservative daily net P&L, unrealised P&L, position count, remaining daily loss budget, observation time, and the exact next five-minute check; unknown evidence is written as `不明`, never zero. The Local `/invest` reader prefers live, then shadow, then paper state and no longer hard-codes `審査中` or `paper loop` after the account is active. Unit evidence passes, but L07 remains open until a main-derived Local shadow natural wake delivers the new message and `/invest` reads that same live snapshot. |
| L08 | Local live shadow — **ACTIVE** | Main-derived release `6c21b354c0ac0100f0b34969986fc17de9da303d` installed separate paper and live-shadow launchd labels with isolated state and a 300-second cadence. The first natural shadow wake made zero broker effects and exposed a paper-campaign coupling at `campaign_read`: the legacy SPY campaign correctly rejects non-paper input, so the pass exited `78` and sent a failure report. The hotfix removes that call path entirely outside paper and derives only a read-only non-paper observation summary. L08 remains open until the hotfix release produces two successful natural shadow wakes and two Telegram message IDs. |
