...
23. [ ] `APPLY-CROWDWORKS-4` Run the shared fitness judge through a supported Apply task class.
    ...
    `application-intent-planner`, the same task family already used by Apply, and 57 shared plus
    CrowdWorks focused tests pass. PASS remains merge, targeted release, a natural wake that no
    longer records this CLI error, one official application/Telegram receipt, and replay-zero.
