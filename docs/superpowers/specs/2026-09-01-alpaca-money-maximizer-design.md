...
| L14 | Ownership transfer and cloud shadow parity — **ACTIVE** | Cloud now packages the official Alpaca CLI `v0.0.14` with pinned checksums, byte-identical shared Telegram client/outbox, and the byte-identical Python strategy/risk/report core. The same allocation prompt/schema swaps only its hosting provider to the Cloud Gemini adapter; no strategy judgment is hardcoded in JavaScript. A service-role-only, tenant-keyed runtime bundle rejects raw credentials, unknown or missing files, cross-tenant rows, oversized payloads, semantic-invalid control/risk/receipt/SQLite state, digest drift, and any cutover proof lacking the ordered `local stopped → queues drained → broker reconciled` timestamps and source release SHA. The exporter/importer carries the live account hash binding, explicit pause/kill state, loss-day baseline state, receipt ledger including unresolved intents and issued client IDs, live position ownership marker, and Telegram outbox/latest receipt with private modes; observation caches and credentials are excluded. Cloud shadow uses one durable tenant directory on a required Railway volume, never reimports an older DB snapshot once the cutover marker exists, and a send-window crash regression proves the retained state survives the next wake. Five-minute job IDs are stable across restart, older claimed slots retain their own lineage, shadow order calls stay zero, and one Telegram report is required before completion. Cloud/L14 tests pass `30/30`, Investment tests pass `121/121`, Local/Cloud parity passes `3/3`, diff checks pass, commits through `e0ac35a55` are pushed, and fresh read-only review returns `ship` with no critical/high blocker. Before the first cloud wake, still stop local and read back no running pass, drain both queues, reconcile official orders/positions, persist and rehydrate the production bundle, attach/read back the durable volume, then enable cloud shadow for at least two natural 300-second wakes. |
