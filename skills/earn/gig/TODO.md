...
| Mercor | **Matched application replay-zero passed; material-low repair is installed and awaiting natural acceptance.** PR `#5004` adds explicit shared high/medium/low definitions. Natural run `18d423d23f7c5250-80399` officially submitted the medium-fit `Software Engineer, Python — Codebase Q&A` listing with Telegram application receipt `75661`; following run `18d4261d3ee9d360-30246` kept that application and Telegram event at exactly one. That replay wake also exposed and submitted the low-fit `Software Engineer, Go — Codebase Q&A` listing with Telegram `75740`. PR `#5005`, merged as `fe149d2d`, now makes material required language/location/domain/seniority contradictions `no_reasonable_shot` and non-submittable, while missing/preferred evidence remains eligible medium. Immutable release `20260911T124425-fe149d2d` is loaded for Mercor Apply only with install receipt `4899367d369548f040317d56`; a natural wake must still prove the boundary without blocking qualified work. | **Accepted.** Consecutive natural passes cover the authenticated in-platform/application event inventory; email is not treated as a platform reply. | **Only official empty-contract monitoring is proved.** Real task preparation, human handoff when physically required, submission, acceptance and payout remain unaccepted. | **Not applicable on current official surface.** | Applications exist, but no accepted paid work or payout is proved. |
