...
| L08 | Local live shadow — **DONE** | Separate main-derived paper and live-shadow launchd labels were proven at 300 seconds with isolated state; shadow has `effect_class=none`, cannot call `submit_order`, and bypasses the paper-only SPY campaign. The first deployed attempt failed closed at that legacy coupling and reported message `71989` with zero broker effects. After the fix, at least two natural shadow wakes exited zero and delivered `72003`, `72026`, and then final-UX `72050`; official readback remained one `USDCUSD` position, zero orders, and no FILL activity. Once the live-account shadow became the owner-facing stream, the legacy paper label was stopped after it emitted a simultaneous `allocator_read` error beside a successful shadow decision. That stop was not durable because both labels remained scheduled in the registry. PR `#4860`, main merge `8db8980c7`, removed the paper owner from managed loops and added `ai.anicca.alpaca-investment` to `retired_labels`; target-only production retirement removed its plist without reloading shadow or another loop. Final readback reports paper `retired/unloaded`, no paper process or plist, shadow `loaded-idle` with exit code zero, and doctor `ok=true`. Paper state is preserved; owner-facing periodic reporting is now durably the single live-account shadow stream. |
...
| L11 | Local 24/7 repeatability — **ACTIVE (owner-directed live execution; preserved shadow evidence: 2/30 NY days, 86/100 natural wakes)** | Dais explicitly ended the remaining shadow wait at `86/100` and directed bounded recurring live execution; the incomplete shadow sample remains evidence and is not relabelled complete. PR `#4868`, main merge `3377d4a39`, added recurring `BTC/USDC` live entry and model `HOLD/EXIT` judgment behind deterministic `$100` allocated-capital, `$10` proposed-loss, worst-case `$20` New-York-day loss, exact owned-quantity, exclusive effect, stable client-ID, reconciliation, pause/kill, and Telegram fences. Fresh read-only review found and the implementation closed proposed-loss overshoot, unrelated-position liquidation, overlapping-wake submission, stale quote, crash ownership, rejected/partial close, and stale close-ID paths; final review returned PASS. Production exposed two pre-effect failures: Apple Python 3.9 evaluated `dict | None` at import (PR `#4870`, merge `0b9bfefe4`) and the generated live plist aliased paper/live state (PR `#4872`, merge `523d7a9e2`). Both runs stopped before broker/model effects and delivered failure reports. Immutable release `20260910T172804-523d7a9e` then ran live successfully: first pass established the daily baseline and delivered Telegram `73566`; the second pass reported `risk_day_ready=true`, official daily P&L `-$0.031256`, unresolved intents `0`, chose model `NO_TRADE`, and delivered `73569`; the following natural 300-second wake `18d3e8bc44c69ac8-63860` ended pass/exit `0`, again chose `NO_TRADE`, submitted no order, and delivered `73578`. `ai.anicca.alpaca-investment-shadow` is unloaded, so only the local live label owns the account. This proves live scheduling/reporting and trade eligibility, not profit or a new fill. The original repeatability gate and later performance gate remain incomplete; bounded live natural wakes now supply subsequent evidence without raising capital. |
