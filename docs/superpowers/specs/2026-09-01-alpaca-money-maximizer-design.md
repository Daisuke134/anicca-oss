...
| L11 | Local 24/7 repeatability — **DONE (100-wake composite window; 86 frozen shadow + 14 stable live)** | Dais explicitly ended shadow observation at `86/100`; that exact terminal boundary is frozen at `2026-09-10T07:30:37.610138Z`, and recurring-live qualification begins at the first stable outer execution `2026-09-10T08:29:27.449781Z`. PRs `#4885` and `#4888`, merges `52bfba0ab` and `54aae2f69`, combine only outer `shared-agent-runner` events across the retired shadow and active live state namespaces, exclude inner provider reports, require both non-overlapping transition boundaries, and correlate exactly one delivered Telegram record inside each execute/report window. The five pre-qualification report gaps and all four failed live bootstrap runs remain in their original ledgers and are not relabelled successful. Main-derived immutable release `20260910T185025-54aae2f6` reproduced `status=pass`: 100 qualifying natural wakes (86 shadow + 14 live), Telegram `100/100`, unreported `0`, unique runtime events, one terminal per run, multiple processes, shadow effects `0`, live unresolved intents `0`, and official duplicate client/order IDs `0`; official loop orders remained `2`. Its following natural wake ended pass/exit `0` on the installed SHA at `2026-09-10T09:57:36.258107Z`, delivered Telegram `73768`, and advanced the still-passing window to `101/101` (86 + 15) with no new order. Account readback remained equity `$66.72`, cash `$0`, one `USDCUSD` position, and official daily P&L `-$0.031256`; the model chose `NO_TRADE` because weak direction and spread did not establish positive expected value. This proves the ordered repeatability/reporting/duplicate/restart path, not profit. |
| L12 | Local performance gate — **DONE** | PR `#4899`, merge `f546f8401`, and immutable release `20260910T200007-f546f840` add a fail-closed official performance adapter and projection. It re-reads and correlates the one completed live round trip using exactly two filled orders, two order-bound FILLs, two order-bound CFEEs, one completed incoming USDC transfer, the single current USDC position, historical executable-side quotes at both fills, and start/end BTC/USDC benchmark quotes. Production `performance-latest.json` is mode `0600` and measures net P&L `-$0.04`, comprising realised `-$0.01` (unrounded `-$0.006967...`) and unrealised `-$0.03`; explicit fees `$0.01`, adverse executable-side slippage `$0.00`, official-period-endpoint drawdown `$0.04` (not claimed as intra-period maximum), peak BTC exposure `$1.96`, benchmark return `-0.000216`, and alpha `-$0.02`. Owner funding is principal, not profit. With one negative round trip, `statistically_supported=false`, `capital_expansion_allowed=false`, and funded capital/exposure above `$100` fail closed while further bounded measurement remains enabled. Focused tests pass `7/7` on Python and Apple Python, Investment tests `120/120`, shared loop tests `366/366`, CI all pass, and the fresh independent review returns `ship`. The installed/event SHA matched `f546f8401` on the first natural post-deploy wake, which ended pass and delivered Telegram `73918`; the model made no order because current BTC/ETH evidence did not overcome the spread. |
| L13 | Disabled cloud wiring — **DONE** | PR `#4907`, main merge `a0dab7f565`, and canonical Railway `Anicca/life-call` deployment `0afb1c77-b22a-4ebf-8720-2776361f2728` package the byte-identical 17-file L12 Python core inside the Railway app root and remove the JavaScript parity strategy fork. The Cloud worker invokes that Python core behind the existing tenant-scoped runtime queue, content-addresses the artifact as `334e84bb74b2df44cd5c9ebd9824cab1b110a407206ba96d783ee33bac52506b`, and preserves the sealed parity `core_digest=85ac90e0027acd0c57ac0a81ef01db1a02c8a2b5b59ae50774fe2a40ec89f259`. Tenant mismatch rejects before enqueue; the Cloud vault adapter requires the exact tenant plus Alpaca key, Alpaca secret, and Telegram token and returns no secret in health/readback. The stale Cloud dry-run flag was removed before the canonical redeploy. Production SSH readback from `life-call-production.up.railway.app` reports `wired_disabled`, schedule false, broker mutation false, Telegram transport wired but delivery false, durable receipts true, and secret health false because credentials intentionally remain Local until L14 ownership transfer. Deployment reached `SUCCESS`, exact build is `a0dab7f5656b0c37ea849c44b269f9c6bb83a4ab`, and HTTP health is `ok=true`. An earlier readback against the separate Railway project named `life-manager` was rejected as non-authoritative and is not acceptance evidence. Node regressions pass `86/86`, Local/Cloud parity `3/3`, Investment tests `121/121`, CI all pass, and fresh review returns `ship`. |
| L14 | Ownership transfer and cloud shadow parity — **DONE** | PRs `#4919`, `#4920`, `#4928`, `#4930`, and `#4932` package the official Alpaca CLI `v0.0.14`, byte-identical Python strategy/risk/report core, shared Telegram outbox, tenant-scoped secrets/state, sealed live-account binding, and durable Railway volume. Production cutover stopped Local after a passing idle wake, drained the Cloud queue, verified Alpaca `ACTIVE`, zero blocks, zero open orders, one `USDCUSD` position, zero unresolved intents, and matching volume/DB bundle digests. Two distinct natural Cloud slots completed with `effect_permission=none`, `order_calls=0`, `message_calls=1`, Telegram message IDs `1242` and `1243`, and persisted outbox rows for both IDs; post-run official readback remained zero open orders and SQLite integrity `ok` with zero pending messages. The first production attempts exposed and fixed three real fail-closed defects before any effect: wrong Telegram directory source, conflicting generic/shadow state variables, and Gemini's rejection of JSON Schema `additionalProperties`; strict local extra-property validation remains. Railway ran the accepted core from latest main including merge `b3dbbb8a0`, and Local live was restored after Cloud shadow acceptance so no permanent shadow owner remains while L15 is implemented. |
| L15 | One cloud parity canary — **DONE** | PR `#4944`, merge `8b58e4f92c755b3e15aa879bb79a9924ef60f89e`, and Railway deployment `a2736a40-941d-4e33-a5cb-7818a314b60a` add a deployment-specific Cloud canary identity, monotonic durable ownership, common writer fencing, fence-scoped broker observation, and canonical handling of Alpaca's three valid BTC/USDC symbol forms while preserving the L09 Local identity and output. Before the effect, production readback proved Cloud shadow disabled, control resumed, zero unresolved intents, Local's final pass successful and then `launchd_state=unloaded`, Alpaca `ACTIVE`/crypto `ACTIVE`, zero blocks, zero open orders, and only the existing USDC position. The first two attempts failed closed before submission because the BTC quote exceeded the unchanged 30-second freshness limit. A later fresh quote allowed exactly one Cloud `$2.00 BTC/USDC` order under the unchanged `$100/$10/$20` policy; official Alpaca readback showed one filled order, zero open orders, one BTC position, `started=1`, `outcome=1`, and Cloud ownership `open`. Immediate replay returned `submitted_this_run=false` and created no second order. Telegram milestone message ID `74404` was delivered. Python Investment tests pass `130/130`, relevant Cloud/packaging Node tests pass `10/10`, all CI checks pass, and fresh money-safety review returns `ship`. |
| L16 | Bounded cloud live campaign — **DONE** | PR `#4955`, merge `95398ecb7f482df037e43ceb5ab6863a66bfc0d6`, and Railway deployment `f282425c-f3ed-4fad-b62f-98ecb9320a36` add the disabled-by-default five-minute Cloud live scheduler with exact tenant/mode/account/state/artifact binding, one-attempt live jobs, deterministic effect IDs, durable failure receipts, volume-derived input/output digests, Telegram job-key dedupe, opposite-mode environment removal, and guaranteed temporary-credential cleanup. Local remained `launchd_state=unloaded`; production moved through `live + paused`, read back `live_enabled` with Vault/state/Telegram healthy, then unpaused. The first natural wake failed closed before order at `allocation_decide` and delivered Telegram `1244`, exposing that the Cloud runner admitted allocation but not the shared position-manager label. Hotfix PR `#4958`, merge `0fb8147717c9783038c336d5e1ad6bd8db53ad82`, and exact Railway deployment `575ed06e-4f14-418d-861f-389b6d65c4a2` admit only `alpaca-allocation` and `alpaca-position`. Two consecutive post-fix natural wakes completed on attempt one with `mode=live`, `effect_permission=money`, zero new orders, `position://BTCUSD` HOLD decisions, matching persisted state digests, and delivered Telegram messages `1247` and `1248`. Official Alpaca readback after both showed zero open orders, the one owned BTC position plus USDC, and positive BTC unrealised P&L at that observation; this is not a profit guarantee. Python Investment tests pass `130/130`, relevant Node tests `58/58`, all CI checks pass, and both fresh money-safety reviews return `SHIP`. |
| L17 | OSS self-host release — **DONE** | PR `#4963`, merge `e98c535acf46c4eaf54bb5376893b646a27ce79e`, and Railway deployment `50491777-7956-4772-aec2-5ac08b456b38` publish a clean-host Docker Compose package on public `main`. It uses the byte-identical Local/Cloud Python reporter and shared decision, risk, receipt, reconciliation, Telegram, and pinned Alpaca CLI paths; defaults to real-account read-only `shadow`; requires `I_UNDERSTAND_REAL_MONEY=true` for `live`; runs non-root with a read-only filesystem, dropped capabilities, tmpfs credentials, and a durable named state volume; and freezes an exact five-minute start-to-start next-wake time for truthful reports. The public README and package guide include architecture, Local/Cloud ownership choice, Telegram example, `$100/$10/$20` risk policy, pause/resume/kill and recovery, secret handling, and threat model without credentials, private account IDs, or a profit guarantee. A clean named volume connected to the real ACTIVE account produced three natural shadow wakes, zero effects, zero unresolved intents, and Telegram messages `74610`, `74622`, and `74633`; Cloud was paused only for this single-writer test, then read back `live`, `paused=false`, `killed=false`, `active` before the main-derived deployment reached `SUCCESS`. Python Investment tests pass `131/131`, relevant Cloud/packaging Node tests `18/18`, packaged image tests `18/18`, OSS/PII/startup checks pass, all CI checks pass, and fresh safety review returns `SHIP`. |
