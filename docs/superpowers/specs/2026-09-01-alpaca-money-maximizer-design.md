...
| L06 | Frozen live-risk gate — **ACTIVE** | The first live snapshot exposed a real accounting defect: completed Solana USDC funding was absent from `CSD/CSW` and was incorrectly projected as `$66.742977` realised profit. The first patch is rejected before merge because it used crypto quantity rather than the provider's transfer-time `usd_value`, grouped by creation rather than first observed completion, and reused `last_equity` instead of the required New-York-midnight baseline. The replacement persists the first official equity read and transfer-status ledger for each New York day, adjusts only verified post-baseline cash flow, and rejects entry while either daily-loss measure is unknown. No order has been submitted. L06 remains open until both required loss measures have official evidence and live readback passes. |
