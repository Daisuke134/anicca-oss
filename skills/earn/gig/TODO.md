...
| CrowdWorks | **Working for fixed-price and hourly applications.** Natural release `eee05950` retained earlier fixed-price proposal `305126036`, reconciled hourly proposals `305130945` and `305132604`, then submitted hourly proposal `305134017` with exact official rate/30-hour readback. Telegram messages `71454`, `71455`, `71463` are delivered, pending is empty, and the following natural PASS kept all three receipt counts at one while submitting a different job. Fixed-price/one-off work remains equally eligible; hourly support extends rather than replaces it. The live contract offer below is downstream evidence that Apply created an opportunity. | **Two official contracts exist; receipt convergence still open.** Shared Reply owns the universal `accept_contract` lifecycle while the thin CrowdWorks adapter owns exact terms, checkbox and provider mutation. Exact message URLs now redirect to contracts `63570481` (Effect, work may start) and `63568785` (JPY 110, escrow pending). Release `b5231f74` replays 21 ordinary threads at zero but still rejects both redirects because navigation only admits proposal URLs. The bounded adapter fix admits exact proposal/contract routes and derives the contract receipt from the same message redirect plus matching title, amount and available parties; related tests pass 304/304. | **Not implemented.** The storefront owner remains disabled, so there is no listing, inquiry, order or revenue receipt chain. | **Existing empty-inventory acceptance is stale; two contracts must now be ingested.** Contract `63570481` may start work; contract `63568785` must remain waiting for escrow. Paid must observe both exact identities, never work before escrow, and own delivery/payment through replay-zero. |
