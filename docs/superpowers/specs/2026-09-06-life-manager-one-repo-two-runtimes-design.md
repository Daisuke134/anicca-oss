...
- [ ] `ARCH-11` Remove external checkout/worktree/OpenClaw/Hermes code dependencies and move required source into this repository. The unused 763 MiB `life-manager-eliza-money-live-report` migration copy is deleted after confirming it was not a Git checkout and had no launchd reference, repository reference or runtime PID. X402 release dependencies and Affiliate publication source now resolve from the canonical repository; unused OpenClaw maintenance loops and the external skills-sync loop are retired. The shared Telegram sender, Capafy reporting and token daily report now call the repository-owned Python Bot API client, with credentials and Capafy state under `~/.local/state/life-manager`. `daily-nl-report` now loads that same environment file and executes its checked-in report source instead of `~/.openclaw` and `~/.anicca/skills`. Other external dependencies remain to be migrated before this atom can close; the currently owned Coconala runtime remains protected from this cleanup pass.
