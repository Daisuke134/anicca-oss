...
| L09 | One local owner-capital canary — **ACTIVE** | Live shadow proved the deposited `USDCUSD` was incorrectly occupying the one-risk-position slot and `$0` USD cash made the reserve gate reject forever. The allocator replacement treats verified USDC funding as cash-like principal, excludes it from committed-risk allocation/position count, and offers the provider-confirmed tradable `BTC/USDC` and `ETH/USDC` pairs. Tests pass; next is an independently reviewed one-shot live-effect fence, followed by one smallest broker-valid order with one stable `client_order_id` and official order/FILL/position readback. |
