...
- [ ] `ARCH-08` Adapt file/JSONL, SQLite and Postgres persistence behind the common contracts. Put Moneytree and every earning-provider receipt behind Financial Manager adapters. Repair `life-manager-cfo-hourly`: its registry and wrapper are on `main`, but the referenced `cfo-hourly-local.js` implementation exists only on the unmerged `feature/cfo-ops3a-canonical` branch, while the configured 2026-08-22 partial-release path also lacks that file; the current targeted pass therefore fails before collecting data. Bring only the required implementation into the canonical repository release, remove the obsolete partial-release override, refresh stale Moneytree evidence, and consolidate the useful Daily/Weekly marketing views without creating a second revenue aggregator.
  ...
  - [ ] FinancialRecord persistence: add one repository-owned, tenant-scoped append/read boundary for common `FinancialRecord` values. Moneytree, Agent Economy and Marketplace adapters must write through it; Financial Manager must read through it. This is the missing shared boundary, not a second revenue ledger.
  - [ ] CFO source repair: implement `cfo-hourly-local.js` over the common FinancialRecord boundary, repository-owned snapshot/delivery stores and existing shared Telegram outbox. Do not restore the old branch closure: rejected PR #4506 proved it depended on repository-unmanaged Supabase RPCs and directly re-aggregated legacy `lm_agent_earnings`.
  - [ ] Installed CFO repair: read back the current launchd environment, remove only the obsolete partial-release override, reconcile the one CFO label from a main-derived immutable release, and verify its targeted receipt.
  - [ ] Moneytree evidence: perform a fresh provider read and attach truthful evidence before changing `verification.status` from `unverified`.
  - [ ] Reporting consolidation: fold the useful Daily/Weekly product views into Financial Manager and retire their redundant aggregation/no-data messages without touching the four running Coconala owners.
