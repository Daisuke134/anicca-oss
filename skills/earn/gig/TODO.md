...
| Mercor | **Fit repair produced a live matched application; replay gate open.** PR `#5004` adds explicit shared high/medium/low definitions so general AI overlap cannot hide a material seniority/language/location/domain contradiction; low-fit person-bound roles no longer page Dais, while ready no-human roles remain eligible after higher-value work. Target install receipt `a45bc71de1b099610e06bc5a` points only Mercor Apply at main release `92cf6b50`. Its first natural run `18d423d23f7c5250-80399` ended PASS/exit `0` and officially submitted `Software Engineer, Python — Codebase Q&A` as `list_AAABoDfKu3fzN7VjWXxDNakz`; the ranking records verified AI-agent/Databricks overlap and explicitly lowers confidence for unverified 3+ years/Python-internals experience. Telegram application receipt `75661` and pass receipt `75664` were delivered once, with zero human gates. The following natural replay-zero remains the active gate. | **Accepted.** Consecutive natural passes cover the authenticated in-platform/application event inventory; email is not treated as a platform reply. | **Only official empty-contract monitoring is proved.** Real task preparation, human handoff when physically required, submission, acceptance and payout remain unaccepted. | **Not applicable on current official surface.** | Applications exist, but no accepted paid work or payout is proved. |
...
The active atom is **Mercor Apply matched-submission replay-zero**. Lancers project
...
first natural run `18d423d23f7c5250-80399` passed and submitted the matched
`Software Engineer, Python — Codebase Q&A` listing with official readback, Telegram application
receipt `75661`, pass receipt `75664`, and no human gate. The remaining gate is the next natural
30-minute wake proving the same listing is not resubmitted or re-reported.
