...
| L13 | Disabled cloud wiring — **ACTIVE** | With cloud scheduling and broker mutation disabled, wire the L12-frozen core artifact behind the existing Life Manager cloud queue, scheduler, encrypted secrets, durable receipts, and Telegram transport. Cloud readback reports the same core digest; the change is host wiring only and introduces no strategy, risk, effect, or reporting fork. |
