...
STEP7 REPORT — MANDATORY every pass. Send through the repo-owned Python client at $LIFE_MANAGER_REPO/skills/_shared/telegram.py:
  (a) the VIDEO itself as a document: python3 $LIFE_MANAGER_REPO/skills/_shared/telegram.py --chat-id "\${CAPAFY_TELEGRAM_TARGET:-\$TELEGRAM_ALERT_CHAT_ID}" document <the mp4 path> --caption "<caption below>". If the video attach fails, send a thumbnail/first-frame with the same client.
