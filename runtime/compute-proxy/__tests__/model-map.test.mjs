...
import { normalizeModelSelection, normalizeRequestBody } from '../model-map.mjs';
...
test('telemetry selection reports the tier paired with the normalized model', () => {
  assert.deepEqual(normalizeModelSelection('auto', 'anthropic/claude-sonnet-4-6'), {
    model: 'anthropic/claude-sonnet-4-6', tier: 'frontier',
  });
  assert.deepEqual(normalizeModelSelection('nvidia/llama-4-maverick', 'unused'), {
    model: 'nvidia/llama-4-maverick', tier: 'free',
  });
  assert.deepEqual(normalizeModelSelection('free/glm-4.7', 'unused'), {
    model: 'nvidia/gpt-oss-120b', tier: 'free',
  });
});
